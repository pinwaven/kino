#include "crc.h"
#include "esp_log.h"

static const char *TAG = "CRC";

static const uint8_t crc8_table[256] = {
    
};

static uint8_t bit_reverse(uint8_t b) {
    b = (b & 0xF0) >> 4 | (b & 0x0F) << 4;
    b = (b & 0xCC) >> 2 | (b & 0x33) << 2;
    b = (b & 0xAA) >> 1 | (b & 0x55) << 1;
    return b;
}

uint8_t cal_crc8(const uint8_t *data, uint16_t len) {
    uint8_t crc = 0x00;
    for (uint16_t i = 0; i < len; i++) {
        crc = crc8_table[crc ^ bit_reverse(data[i])];
    }
    return crc;
}

static uint8_t cal_crc8_math(const uint8_t *data, uint16_t len) {
    uint8_t crc = 0x00;
    for (uint16_t i = 0; i < len; i++) {
        uint8_t byte = bit_reverse(data[i]);
        crc ^= byte;
        for (int j = 0; j < 8; j++) {
            if (crc & 0x80) {
                crc = (crc << 1) ^ 0x07;
            } else {
                crc <<= 1;
            }
        }
    }
    return crc;
}

bool crc_verify_equivalence(void) {
    for (int i = 0; i < 256; i++) {
        uint8_t data = (uint8_t)i;
        uint8_t math_res = cal_crc8_math(&data, 1);
        uint8_t lut_res = cal_crc8(&data, 1);
        if (math_res != lut_res) {
            ESP_LOGE(TAG, "CRC Equivalence verification FAILED at 0x%02X: math=0x%02X, lut=0x%02X", i, math_res, lut_res);
            return false;
        }
    }
    ESP_LOGI(TAG, "CRC Equivalence verification PASSED for all 256 byte values!");
    return true;
}
